...
describe("patch-notes search entries", () => {
  test("builds localized patch-note entries with change kind tokens", () => {
    ...
    expect(items[0]).toMatchObject({
      kind: "patch-note",
      patch: "26.13",
      href: patchNoteSectionHref("26.13", "augments"),
      name: "版本 26.13 · Augments",
      snippet: "阿塔瑪的清算: 新增一個會依裝備成長的增幅。 英雄平衡: 修正提示文字不一致。",
    });
    expect(items[0].href).toBe("/patch-notes/26.13#patch-26-13-augments");
    expect(items[0].href).toContain(
      `#${patchNoteSectionAnchor("26.13", "augments")}`,
    );
